...
def ai_learn(mean_over, features=None, scaler=AI_SCALER.AI_NONE):
    ...
    with parallel_backend("multiprocessing", n_jobs=multiprocessing.cpu_count() - 2):
        ...
        models = [ElasticNet(), Lasso(), Ridge(),
                  ...
                  ExtraTreesRegressor(), SVC(gamma='scale')]
        last_estimation = 0
        for _ in range(len(models)):
            ...
            if current_name == 'elasticnet':
                search = HalvingGridSearchCV(current_model, param_elastic, cv=cv, verbose=3)
                current_model = search
            elif current_name == 'randomforestregressor' or current_name == 'extratreesregressor' or current_name == 'decisontreeregressor':
